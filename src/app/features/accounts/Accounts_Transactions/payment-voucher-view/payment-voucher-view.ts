...
export class PaymentVoucherView implements OnInit, OnDestroy {
  ...
  getPartyJournalEntryData(): void {
    try {
      ...
      const ledgerNames = [
        ...new Set(
          this.paymentsList.map((p: any) => p.pledgername).filter(Boolean)
        ),
      ];
      ...
      for (const ledger of ledgerNames) {
        ...
        const sections = [...new Set(ledgerPayments.map((p: any) => p.pTdsSection).filter(Boolean))];
        for (const sec of sections) {
          const tdsAmt = ledgerPayments
            .filter((p: any) => p.pTdsSection === sec)
            .reduce((s: any, p: any) => s + safe(p.ptdsamount), 0);
          if (tdsAmt > 0) {
            tdsEntries.push({
        ...
        const totalTds = ledgerPayments.reduce(
          (s: any, p: any) => s + safe(p.ptdsamount),
          0
        );
        if (totalTds > 0) {
          tdsEntries.push({
      ...
      [
        ...
        const total = (this.paymentsList || []).reduce(
          (s: any, p: any) =>
            s + Number(this.commonService.removeCommasInAmount(p[gst.key]) || 0),
          0
        );
        if (total > 0) {
          this.partyjournalentrylist.push({
