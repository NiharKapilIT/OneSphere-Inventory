...
export class InventoryScreenShell implements OnInit {
  ...
  protected readonly partyContactOptions = computed<GlobalContactOption[]>(() => {
    if (this.config?.key !== 'vendorMaster' && this.config?.key !== 'customerMaster') return this.globalContacts;
  ...
  private persistNewAttributeValueIfNeeded(attributeName: string, value: any): void {
    ...
    const newOnes = tokens.filter(token => !existing.some(option => this.optionEquals(option, token)));
    ...
    }, attribute.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(res => {
      if (res.success && res.data) {
        const saved = res.data as AttributeItem;
        this.loadedAttributeObjects.update(items => items.map(item => item.id === saved.id ? saved : item));
        this.allAttributeObjects.update(items => items.map(item => item.id === saved.id ? saved : item));
      }
    });
  }
  ...
  private static readonly QUICK_ADD_MASTERS_WITH_AUTO_CODE = new Set([
    'Variant', 'UOM', 'Serial Number Policy', 'Batch / Lot Policy', 'Brand', 'Product Type'
  ]);
  ...
  saveQuickVendor(): void {
    ...
    this.isSavingQuickAdd.set(true);
    this.quickAddError.set('');
    this.inventoryConfigService.saveVendor({
      segment_id: this.selectedSegmentId(),
  ...
  procurementVendorHeaderField(): InventoryField | null {
    ...
    if (!procurementKeys.has(this.config?.key || '')) return null;
  ...
  procurementVendorHeaderOptions(field: InventoryField): string[] {
    return field.options?.length ? field.options : this.vendorOptions;
  }
  ...
  private referenceDocsForType(type: string, docs: PurchaseRefDoc[]): PurchaseRefDoc[] {
    ...
    if (type === 'SI')  return docs.filter(doc => this.normalizeKey(doc.status) === 'posted');
      ...
      this.txService.getReservedSerialsForDcItem(dcItemId)
        ...
        .subscribe({
          next: res => {
            ...
            const serials = options.map(s => s.serial_no);
